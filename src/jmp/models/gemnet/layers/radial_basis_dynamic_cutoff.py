...
class GaussianBasis(nn.Module):
    def __init__(
        ...
        if self.trainable_stds:
            self.temps = nn.Parameter(torch.empty(num_gaussians), requires_grad=True)
            temp_mean = -0.5 / ((stop - start) / (num_gaussians - 1)) ** 2
            _ = nn.init.normal_(self.temps, mean=temp_mean, std=abs(temp_mean) * 2.0)
        else:
...
class RadialBasis(nn.Module):
    ...
    def forward(self, d, *, data: Batch):
        if not (cutoff := self.absolute_cutoff):
            edge_index = self._get_tensor(data, f"{self.graph_type}_edge_index")
            cutoff = self._get_tensor(data, f"{self.graph_type}_cutoff")  # (b,)
